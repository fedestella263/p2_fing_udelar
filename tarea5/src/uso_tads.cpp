...
cadena_t mayores(cadena_t cad, unsigned int k, unsigned int max) {
    ...
    cadena_t cad_res = crear_cadena();
    
    cola_prioridad_t cp = crear_cp(k, max);
    
    while(es_localizador(loc)) {
        
        if((unsigned int)numero_info(info_cadena(loc, cad)) <= max) {
            if(k > 0) {
                // printf("%d\n", k);
                if(!hay_prioridad(numero_info(info_cadena(loc, cad)), cp)) {
                    insertar_en_cp(copia_info(info_cadena(loc, cad)), numero_info(info_cadena(loc, cad)), cp);
                    k--;
                    faso++;
                }
            } else if((unsigned int)numero_info(info_cadena(loc, cad)) > prioridad_prioritario(cp) && !hay_prioridad(numero_info(info_cadena(loc, cad)), cp)) {
    ...
    while(faso) {
        ...
        insertar_al_final(copia_info(prioritario(cp)), cad_res);
        eliminar_prioritario(cp);
        faso--;
    }
